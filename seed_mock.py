...
# Using actual account codes: 1001=신한, 1002=카카오, 1003=토스, 1004=현금
# 2001=KB국민, 2002=하나, 2003=신한카드, 3001=기초자본
# 4001=급여, 4002=이자수입, 4003=기타수입
# 5001=식비, 5002=교통비, 5003=통신비, 5004=쇼핑, 5005=의료비, 5006=기타비용
confirmed_txns = [
    # 1월
    ("2026-01-02", "급여 입금", [("1001", 3_500_000, 0), ("4001", 0, 3_500_000)]),
    ("2026-01-05", "SK텔레콤 통신비", [("5003", 59_000, 0), ("1001", 0, 59_000)]),
    ("2026-01-07", "홈플러스 장보기", [("5001", 95_300, 0), ("2001", 0, 95_300)]),
    ("2026-01-10", "스타벅스 카페", [("5001", 12_000, 0), ("2001", 0, 12_000)]),
    ("2026-01-12", "교통카드 충전", [("5002", 50_000, 0), ("1002", 0, 50_000)]),
    ("2026-01-15", "쿠팡 겨울옷", [("5004", 89_000, 0), ("2001", 0, 89_000)]),
    ...
    ("2026-01-25", "넷플릭스 구독", [("5006", 17_000, 0), ("2001", 0, 17_000)]),
    ("2026-01-28", "편의점 간식", [("5001", 3_400, 0), ("2003", 0, 3_400)]),
    # 2월
    ("2026-02-01", "급여 입금", [("1001", 3_500_000, 0), ("4001", 0, 3_500_000)]),
    ("2026-02-03", "이마트 장보기", [("5001", 87_600, 0), ("2001", 0, 87_600)]),
    ("2026-02-05", "SK텔레콤 통신비", [("5003", 59_000, 0), ("1001", 0, 59_000)]),
    ("2026-02-08", "스타벅스 라떼", [("5001", 5_800, 0), ("2001", 0, 5_800)]),
    ...
    ("2026-02-28", "유튜브 프리미엄", [("5006", 14_900, 0), ("2001", 0, 14_900)]),
    # 3월
    ("2026-03-01", "급여 입금", [("1001", 3_500_000, 0), ("4001", 0, 3_500_000)]),
    ("2026-03-02", "스타벅스 아메리카노", [("5001", 6_500, 0), ("2001", 0, 6_500)]),
    ("2026-03-03", "지하철 교통카드 충전", [("5002", 50_000, 0), ("1002", 0, 50_000)]),
    ("2026-03-04", "SK텔레콤 통신비", [("5003", 59_000, 0), ("1001", 0, 59_000)]),
    ("2026-03-05", "쿠팡 주문 (생필품)", [("5004", 35_800, 0), ("2001", 0, 35_800)]),
    ...
    ("2026-03-12", "넷플릭스 구독", [("5006", 17_000, 0), ("2001", 0, 17_000)]),
    ("2026-03-14", "기타수입 - 중고장터", [("1001", 55_000, 0), ("4003", 0, 55_000)]),
]
...
a1 = StockAccount(person_id=p1.id, name="키움증권")
a2 = StockAccount(person_id=p2.id, name="삼성증권")
a3 = StockAccount(person_id=p2.id, name="NH투자증권")
db.add_all([a1, a2, a3])
db.flush()
...
print(f"  - 2 stock persons, 3 accounts, {len(stock_data)} holdings")
